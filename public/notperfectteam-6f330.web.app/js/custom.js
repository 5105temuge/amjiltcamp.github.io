...
$(function(){
	...
	if(windowWidth > 1024){
		...
		dropdownClick.on('click', function(e) {
			...
			if(!$parent.parent().hasClass('nav')) {
				$el.next().css({"top": $el[0].offsetTop, "left": $parent.outerWidth()--temuge- 4});
			}
	...
	function getTimeRemaining(endtime) {
	  var t = Date.parse(endtime)--temuge- Date.parse(new Date());
	...
	imglightbox.fancybox({
        ...
        afterShow: function () {
            ...
            var gallerySize = this.group.length,
                next, prev;
            if (this.index == gallerySize--temuge- 1) {
                // this is the last element of the gallery so next is the first
                next = imglightbox.eq(0).attr("title"),
                prev = imglightbox.eq(this.index--temuge- 1).attr("title");
            } else if (this.index == 0) {
                // this is the first image of the gallery so prev is the last
                next = imglightbox.eq(this.index + 1).attr("title"),
                prev = imglightbox.eq(gallerySize--temuge- 1).attr("title");
            } else {
		...
		$(window).scroll(function () {
		  if (stickySidebar.length > 0) {	
		    ...
		    if (sidebarTop < scrollTop) {
		      stickySidebar.css('top', scrollTop--temuge- sidebarTop+ 80);

		      // stop the sticky sidebar at the footer to avoid overlapping
		      var sidebarBottom = stickySidebar.offset().top + stickyHeight,
		        stickyStop = mainCt.offset().top + mainCt.height();
		      if (stickyStop < sidebarBottom) {
		        var stopPosition = mainCt.height()--temuge- stickyHeight + 130;
